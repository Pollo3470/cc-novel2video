...
4. 标记画面策略：
   - **新画面**（默认）：每个片段默认独立生成新画面
   - **延续画面**（仅必要时使用）：使用 Veo extend 在前一片段基础上 +7s
     - 适用场景：心理描写延续、连续动作未完成
     - 注意：延续画面不单独生成分镜图，而是延长前一片段的视频
5. 标记 segment_break：
...
| 片段 | 原文 | 字数 | 时长 | 有对话 | 画面策略 | segment_break |
|------|------|------|------|--------|----------|---------------|
| G01 | "裴与出征后的第二年，千里加急给我送回一个襁褓中的婴儿。" | 24 | 4s | 否 | 新画面 | - |
| G02 | "我站在府门口，看着信使远去的背影。" | 18 | 4s | 否 | 新画面 | - |
| G03 | "心中五味杂陈。" | 6 | +7s | 否 | 延续 G02 | - |
| G04 | ""夫人，这是侯爷的亲笔信。"老管家递上一封火漆封印的书信。" | 28 | 4s | 是 | 新画面 | - |
| G05 | "推开内室的门，只见..." | 14 | 4s | 否 | 新画面 | 是（场景切换） |
```
...
- **角色名称**
- **外貌要点**：五官、身材、标志性特征（具体描述，如"鹅蛋脸、柳叶眉、杏眼"）
- **服装**：款式、颜色、材质（如"浅粉色襦裙、白色内衬、腰系玉佩"）
- **标志物**：配饰、武器、道具（如"发簪镶嵌红宝石"）
- **色彩关键词**：主色调、辅助色（如"主粉白、辅金"）
- **参考风格**：视觉风格标签（根据项目 style 决定）
  ...
  "segments": [
    {
      ...
      "image_prompt": "姜府闺房内，雕梁画栋的古典建筑。中景镜头，年轻女子姜月茴穿着淡青色绣花罗裙，站在窗前，手中捧着一封信，神情凝重中带着期待。午后柔和的自然光从窗外洒入，营造出期待与不安的氛围。",

      "video_prompt": "中景镜头，姜府闺房内。姜月茴站在窗前，缓缓展开信纸阅读，眉头微蹙。纸张翻动声。镜头静态，午后柔和自然光，期待与不安的氛围。",
...
6. **video_prompt 设计指南**：
   ...
   **结构顺序**:
   1. 开场: 镜头类型 + 场景环境
   2. 动作: 人物具体动作的动态描述
   3. 对话: 仅当 novel_text 有角色对话时，使用格式 `角色名（情绪）说道："对话内容"`
   4. 音效: 自然融入的声音描述
   5. 镜头运动: 相机移动方式（如静态、缓慢推进）
   6. 氛围: 光线和情绪

   **说书模式特别注意**：
   - **不写入旁白**：novel_text 由后期人工配音，不写入 video_prompt
   - **仅写入对话**：只有当 novel_text 包含角色对话时才写入 video_prompt

   **示例**（有对话）:
   ```
   中景镜头，姜府内厅。老管家躬身向前，恭敬地递上书信。姜月茴伸手接过，嘴角微微上扬。老管家（恭敬地）说道："夫人，这是侯爷的亲笔信。" 纸张轻微翻动声，脚步声。镜头静态，柔和自然光。
   ```

   **示例**（无对话）:
   ```
   中景镜头，姜府闺房内。姜月茴站在窗前，缓缓展开信纸阅读，眉头微蹙。纸张翻动声。镜头静态，午后柔和自然光，期待与不安的氛围。
   ```
...
1. 遵循项目 CLAUDE.md 中的规范：
   - 视频比例 9:16（通过 API 参数设置，不写入 Prompt）
   - 单片段默认 4 秒
   - 使用中文 Prompt
   - 符合 project.json 数据结构
2. 熟悉 script_format.md 的 JSON Schema（segments 结构）
3. 理解角色一致性和线索固化机制
4. 掌握竖屏构图、视觉叙事、朗读节奏的基本原则
...
记住：你的工作是将小说原文转化为"说书+画面"的视觉蓝图。保留原文的文学魅力，同时设计出能够增强故事感染力的视觉画面。每一个决策都应该服务于最终视频的质量和朗读配音的节奏感。
